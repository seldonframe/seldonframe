...
export const TOOLS = [
...

  {
    ...
    description:
      ...
      "(b) `local_file_path` (v1.10.1+) — path to a file UNDER THE CURRENT PROJECT DIRECTORY (the MCP server's working directory). The MCP server (running locally) reads the file and forwards bytes to the backend. v1.61.0: paths outside the project directory are rejected (symlinks resolved first) unless the operator has opted extra directories in via the SELDONFRAME_UPLOAD_ROOTS env var — if a file lives elsewhere (e.g. the desktop), ask the operator to copy it into the project first. file_name + content_type derived from the path. " +
    ...
    inputSchema: obj(
      {
        ...
        local_file_path: str(
          "Path to a file under the MCP server's working directory (the project the operator launched Claude Code in). Absolute or relative; relative resolves against that directory. Paths outside it are rejected (v1.61.0 containment — see tool description). MCP reads the file and forwards bytes to the backend. file_name + content_type auto-derived from the path.",
        ),
        image_data_b64: str(
    ...
    handler: async (args) => {
      ...
      if (args.image_url) {
        ...
        // v1.61.0 — containment BEFORE the read: only files under the MCP
        // server's working directory (or SELDONFRAME_UPLOAD_ROOTS opt-ins)
        // may be read, symlinks resolved first. Byte-sniffing below still
        // applies, but sniffing alone left real images anywhere on disk
        // reachable; containment closes the local-file-exfiltration path.
        let filePath;
        try {
          filePath = assertLocalPathAllowed(args.local_file_path);
        } catch (err) {
          return {
            ok: false,
            error: "path_not_allowed",
            message: `upload_workspace_image: ${err?.message ?? err}`,
          };
        }
        let buf;
        try {
