...
- **The locked tier (canonical in ship-feature):** `haiku` = scout/grader/summarizer/transcription · `sonnet` = prose-brief makers + normal reviews · `fable` (Fable 5, best) = hard builds + the two judgment roles (hot-path/subtle review + final whole-branch review), `opus` swappable (prefer the opus swap when the maker was fable, for blind-spot decorrelation). Always name the model on every dispatch.
