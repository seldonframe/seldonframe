...
// 33%"`) and the step copy is plain text inside a <span>; both are
...
// "Step {step} of 3" emits "Step <!-- -->3<!-- --> of 3" not "Step 3
// of 3" — a literal regex on the visible copy fails. The helper
// builds a regex that tolerates the optional comment markers around
// the interpolated step number.
function stepCounterRegex(n: 1 | 2 | 3): RegExp {
  return new RegExp(`Step (?:<!-- -->)?${n}(?:<!-- -->)? of 3`);
}

describe("<OnboardingShell>", () => {
  test("step 1 renders 33% fill + 'Step 1 of 3' copy", () => {
    const html = renderToString(<OnboardingShell step={1} title="Connect AI" />);
    assert.match(html, /width:33%/);
    assert.match(html, stepCounterRegex(1));
    assert.match(html, /Connect AI/);
  });

  test("step 2 renders 67% fill + 'Step 2 of 3' copy", () => {
    const html = renderToString(
      <OnboardingShell step={2} title="Build your first workspace" />,
    );
    assert.match(html, /width:67%/);
    assert.match(html, stepCounterRegex(2));
    assert.match(html, /Build your first workspace/);
  });

  test("step 3 renders 100% fill + 'Step 3 of 3' copy", () => {
    const html = renderToString(
      <OnboardingShell step={3} title="Make it yours" />,
    );
    assert.match(html, /width:100%/);
    assert.match(html, stepCounterRegex(3));
  ...
  test("progressbar has aria-valuenow matching the fill percentage", () => {
    ...
    for (const [step, expected] of [
      ...
      const html = renderToString(
        <OnboardingShell step={step} title="Test" />,
      );
      assert.match(
        html,
        new RegExp(`aria-valuenow="${expected}"`),
        `step ${step} should set aria-valuenow=${expected}`,
      );
    }
  });

  test("progressbar role + aria-valuemin/max are correctly wired", () => {
    const html = renderToString(<OnboardingShell step={2} title="Test" />);
