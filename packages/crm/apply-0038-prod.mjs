// One-shot applier for migration 0038 (device_auth_requests).
...
const SQL_PATH =
  "C:/Users/maxim/CascadeProjects/Seldon Frame/.claude/worktrees/blueprint-renderer/packages/crm/drizzle/0038_device_auth_requests.sql";
...
console.log(`Applying ${statements.length} statements from 0038_device_auth_requests.sql...`);
...
const cols = await sql`
  ...
  WHERE table_schema = 'public' AND table_name = 'device_auth_requests'
...
console.log(`\n✓ device_auth_requests (${cols.length} columns)`);
const idx = await sql`
  SELECT indexname FROM pg_indexes
  WHERE schemaname = 'public' AND tablename = 'device_auth_requests'
  ORDER BY indexname
`;
