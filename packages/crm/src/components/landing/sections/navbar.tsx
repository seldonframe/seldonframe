...
export function NavbarSection({ businessName, logoUrl, navLinks = [], ctaText = "Book Now", ctaLink = "#" }: NavbarSectionContent) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[hsl(var(--border)/0.65)] bg-background/85 backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4 px-5 py-4">
        ...
        <nav className="hidden items-center gap-4 md:flex">
          {links.map((link) => (
            <Link key={`${link.label}-${link.href}`} href={link.href} className="text-sm text-[hsl(var(--muted-foreground))] hover:text-foreground">
              {link.label}
            </Link>
          ))}
        </nav>
