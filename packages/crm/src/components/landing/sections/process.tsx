          ...
          {steps.map((step, index) => (
            <div key={`${step.title}-${index}`} className="relative">
              <article className="glass-card rounded-2xl p-6 text-center md:p-7">
                ...
                <p className="mt-2 text-base text-[hsl(var(--muted-foreground))]">{step.description}</p>
              </article>
              {index < steps.length - 1 ? (
                <span className="pointer-events-none absolute -right-2 top-1/2 hidden h-px w-4 bg-[hsl(var(--border))] md:block" aria-hidden="true" />
              ) : null}
            </div>
          ))}
        </div>
