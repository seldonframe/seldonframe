// v1.13.0 — POST /api/v1/workspace/v2/intake/fields
//
// One endpoint, four ops via `op` field:
...
//
// Index-based addressing handles duplicate-label edge cases. ID
// uniqueness enforced on add/update.
...
import {
  ...
  moveIntakeFieldForWorkspace,
  deleteIntakeFieldForWorkspace,
  updateIntakeFieldForWorkspace,
  ...
  if (op === "add") {
    ...
    const position =
      typeof body.position === "number" && Number.isInteger(body.position)
        ? body.position
        : undefined;
    result = await addIntakeFieldForWorkspace(
      workspaceId,
      body.field as IntakeQuestion,
      position,
    );
  } else if (op === "move") {
    if (
    ...
    result = await moveIntakeFieldForWorkspace(
      workspaceId,
      body.from_index,
      body.to_index,
    );
    ...
    result = await deleteIntakeFieldForWorkspace(workspaceId, body.index);
  } else {
    // update
    if (
    ...
    result = await updateIntakeFieldForWorkspace(
      workspaceId,
      body.index,
      body.patch as Partial<IntakeQuestion>,
    );
  }

  if (!result.ok) {
    logEvent(
      "v2_intake_field_op_failed",
