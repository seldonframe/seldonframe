  ...
  return (
    <section className="animate-page-enter space-y-4">
      ...
      <article className="rounded-xl border bg-card p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">{block.longDescription || block.description}</p>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            {details.ratings.map((rating) => (
              <li key={rating.id} className="rounded border border-border px-3 py-2">
                ...
                <p className="text-muted-foreground">{rating.review || "No written review"}</p>
              </li>
            ))}
          </ul>
        )}
