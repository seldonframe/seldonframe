    ...
    <section className="animate-page-enter space-y-4">
      ...
      >
        <div className="grid gap-3 md:grid-cols-2">
          <div>
            <label htmlFor="block-name" className="mb-1 block text-sm text-muted-foreground">Block name</label>
            <input id="block-name" name="name" className="crm-input h-10 w-full px-3" required />
          </div>
          <div>
            <label htmlFor="block-slug" className="mb-1 block text-sm text-muted-foreground">Block ID / slug</label>
            <input id="block-slug" name="slug" className="crm-input h-10 w-full px-3" placeholder="courses" required />
          </div>
        </div>

        <div>
          <label htmlFor="block-description" className="mb-1 block text-sm text-muted-foreground">Short description</label>
          <input id="block-description" name="description" maxLength={160} className="crm-input h-10 w-full px-3" required />
        </div>

        <div>
          <label htmlFor="block-long-description" className="mb-1 block text-sm text-muted-foreground">Long description</label>
          <textarea id="block-long-description" name="longDescription" className="crm-input min-h-20 w-full p-3" />
        </div>

        <div className="grid gap-3 md:grid-cols-3">
          <div>
            <label htmlFor="block-icon" className="mb-1 block text-sm text-muted-foreground">Icon</label>
            <input id="block-icon" name="icon" defaultValue="Puzzle" className="crm-input h-10 w-full px-3" />
          </div>
          <div>
            <label htmlFor="block-category" className="mb-1 block text-sm text-muted-foreground">Category</label>
          ...
          <div>
            <label htmlFor="block-price" className="mb-1 block text-sm text-muted-foreground">Price (USD)</label>
            <input id="block-price" name="price" type="number" min="0" step="0.01" defaultValue="0" className="crm-input h-10 w-full px-3" />
          </div>
        </div>

        <div>
          <label htmlFor="block-md" className="mb-1 block text-sm text-muted-foreground">BLOCK.md content</label>
          <textarea
            id="block-md"
            name="blockMd"
