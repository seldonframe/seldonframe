      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {Array.from({ length: 6 }).map((_, idx) => (
          <article key={idx} className="rounded-xl border bg-card p-5">
            <div className="h-5 w-2/3 animate-pulse rounded bg-muted/[0.55]" />
            <div className="mt-3 h-4 w-full animate-pulse rounded bg-muted/40" />
            <div className="mt-2 h-4 w-4/5 animate-pulse rounded bg-muted/35" />
            <div className="mt-4 h-9 w-24 animate-pulse rounded bg-muted/50" />
          </article>
        ))}
      </div>
    </section>
