        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            {pipelineViews.map((candidate) => {
              ...
              return (
                <Link key={candidate.name} href={`/${config.spec.routeBase}/pipeline?${href.toString()}`} className={isActive ? "rounded-[11px] border border-primary/30 bg-primary/15 px-2.5 py-1 text-primary" : "rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 text-primary"}>
                  {label}
                </Link>
              );
            })}
