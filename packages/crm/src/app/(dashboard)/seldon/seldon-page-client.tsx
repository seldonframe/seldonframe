...
function ResultCard({
  ...
  if (action === "update") {
    return <UpdateResultCard result={result} />;
  }

  return <CreateOrPlanResultCard result={result} />;
}
                ...
                {state.results?.length ? (
                  <div className="space-y-3">
                    ...
                    {state.results.map((result) => (
                      <ResultCard key={result.blockId} result={result} action={state.action} onViewBlockMd={setSelectedResult} />
                    ))}
                ...
                {activeSessionResults.length > 0 ? (
                  <div className="space-y-3">
                    ...
                    {activeSessionResults.map((result) => (
                      <ResultCard key={`session-${result.blockId}`} result={result} onViewBlockMd={setSelectedResult} />
                    ))}
                  </div>
                ) : null}
