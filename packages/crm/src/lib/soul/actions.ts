...
export async function updateSoulBusinessProfileAction(input: {
  ...
  // 2026-05-17 — agency-primary workspaces created at signup have
  // soul=null (they never went through URL extraction or the soul
  // wizard). Operators visiting /settings/profile hit "Could not save
  // profile" because the action used to throw here. Now: seed a minimal
  // soul on-the-fly using defaults, then apply the operator's edits.
  // No migration needed.
  const existingSoul =
    (org?.soul as OrgSoul | null) ??
    ({
      businessName: "",
      businessDescription: "",
      industry: "",
      ...
      aiContext: "",
      customContext: "",
      suggestedIntakeForm: { name: "Get a Quote", fields: [] },
      branding: { primaryColor: "234 89% 74%", accentColor: "262 83% 70%", mood: "minimal" },
