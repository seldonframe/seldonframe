...
// ─── take_message ──────────────────────────────────────────────────────────
//
// Voice-only tool. When a call can't be resolved as a booking (complex
// request, non-service call, caller preference to leave a message), the agent
// collects the caller's name, phone, and message, then:
//   1. Writes a "voice_message" activity row to the operator's CRM.
//   2. Sends an SMS notification to agents.blueprint.notifyPhone (the
//      workspace owner's personal phone) if configured — best-effort, never
//      throws. Falls back gracefully when notifyPhone is absent or Twilio is
//      unconfigured.
//
// The activity row lands in /activities under type "voice_message" so the
// operator can see all taken-messages alongside bookings and escalations
// without a separate "messages" table.

const takeMessageInput = z.object({
  callerMessage: z
    .string()
    .min(3)
    .describe("What the caller wants to pass along to the team"),
  callerName: z.string().optional().describe("Caller's name if given"),
  callerPhone: z
    .string()
    .optional()
    .describe("Caller's callback phone number if given"),
});

export const takeMessage: AgentTool<
  z.infer<typeof takeMessageInput>,
  { ok: boolean; smsDelivered: boolean }
> = {
  name: "take_message",
  description:
    "Take a message for the team when the call can't be resolved as a booking — " +
    "complex request, general inquiry, caller prefers a callback. " +
    "Logs the message as an activity in the CRM and SMSes the workspace owner " +
    "if their notify phone is configured. " +
    "Use AFTER collecting the caller's name, phone, and message (one at a time).",
  ...
  jsonSchema: {
    ...
    properties: {
      callerMessage: {
        type: "string",
        description: "What the caller wants to pass along to the team",
      },
      callerName: { type: "string" },
      callerPhone: { type: "string" },
    },
    required: ["callerMessage"],
  },
  execute: async (input, ctx) => {
    if (ctx.testMode) {
      return { ok: true, smsDelivered: false };
    }

    // Lazy imports — same pattern as escalate_to_human.
    const { db: dbInstance } = await import("@/db");
    const { activities, agents: agentsTable, users } = await import("@/db/schema");
    const { eq: eqFn } = await import("drizzle-orm");

    // 1. Load the agent blueprint to get notifyPhone.
    let notifyPhone: string | undefined;
    try {
      const [agentRow] = await dbInstance
        .select({ blueprint: agentsTable.blueprint })
        .from(agentsTable)
        .where(eqFn(agentsTable.id, ctx.agentId))
        .limit(1);
      notifyPhone = (agentRow?.blueprint as { notifyPhone?: string } | undefined)
        ?.notifyPhone?.trim() || undefined;
    } catch {
      // best-effort — proceed without notifyPhone
    }

    // 2. Find the workspace owner for activity attribution (activities.userId
    //    is NOT NULL, so we need a real user id). If none found, skip the row.
    const [owner] = await dbInstance
      .select({ id: users.id })
      .from(users)
      .where(eqFn(users.orgId, ctx.orgId))
    ...
    if (owner?.id) {
      ...
      if (input.callerName) bodyParts.push(`Name: ${input.callerName}`);
      if (input.callerPhone) bodyParts.push(`Phone: ${input.callerPhone}`);
      bodyParts.push(input.callerMessage);
      ...
      await dbInstance
        ...
        .values({
          ...
          contactId: null,
          type: "voice_message",
          subject: `Voice message${input.callerName ? ` from ${input.callerName}` : ""}`,
          body: bodyParts.join("\n"),
          metadata: {
            source: "voice",
            agentId: ctx.agentId,
    ...
    // 3. SMS the owner's notifyPhone (best-effort — skip if not configured).
    let smsDelivered = false;
    if (notifyPhone) {
      try {
        const { sendSmsFromApi } = await import("@/lib/sms/api");
        const smsParts: string[] = ["📞 Voice message"];
        if (input.callerName) smsParts.push(`from ${input.callerName}`);
        if (input.callerPhone) smsParts.push(`(${input.callerPhone})`);
        smsParts.push(`— "${input.callerMessage.slice(0, 120)}"`);
        const body = smsParts.join(" ");

        await sendSmsFromApi({
          orgId: ctx.orgId,
          userId: null,
          contactId: null,
          toNumber: notifyPhone,
          body: body.slice(0, 160), // stay in 1 SMS segment
        });
        smsDelivered = true;
      } catch {
        // best-effort — a missing Twilio config or suppression is expected in
        // workspaces that haven't set up SMS; don't crash the tool call.
        smsDelivered = false;
      }
    }

    return { ok: true, smsDelivered };
  },
};
