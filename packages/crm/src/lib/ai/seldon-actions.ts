...
export type SeldonResultAction = {
  label: string;
  kind: "link" | "prompt";
  href?: string;
  prompt?: string;
...
function buildResultSummary(result: SeldonRunResult) {
  return result.description?.trim() || normalizeSummaryLine(result.summary);
}

function buildStructuredActions(result: SeldonRunResult): SeldonResultAction[] {
  const actions: SeldonResultAction[] = [];

  if (result.adminUrl || result.openPath) {
    actions.push({
      label: "Apply",
      kind: "link",
      href: result.adminUrl ?? result.openPath,
      primary: true,
    });
  }

  if (result.editUrl || result.adminUrl) {
    actions.push({
      label: "Edit",
      kind: "link",
      href: result.editUrl ?? result.adminUrl,
    });
  }
  ...
  actions.push({
    label: "Undo",
    kind: "prompt",
    prompt: `Undo the last change to ${result.blockName}.`,
  });

  if (result.publicUrl || result.adminUrl) {
    actions.push({
      label: "View live preview",
      kind: "link",
      href: result.publicUrl ?? result.adminUrl,
    });
  }
...
function finalizeRunResult(result: SeldonRunResult): SeldonRunResult {
  return {
    ...result,
    previewUrl: result.previewUrl ?? result.publicUrl ?? result.adminUrl ?? null,
    cardSummary: result.cardSummary ?? buildResultSummary(result),
    actions: result.actions ?? buildStructuredActions(result),
  };
}

function finalizeRunResults(results: SeldonRunResult[]) {
  return results.map((result) => finalizeRunResult(result));
}
...
export async function runSeldonItAction(_prev: SeldonRunState, formData: FormData): Promise<SeldonRunState> {
  ...
  if (endClientMode) {
    endClientSession = portalTokenFromForm
      ? await getPortalSessionForToken(orgSlugFromForm, portalTokenFromForm)
      : await getPortalSessionForOrg(orgSlugFromForm);
    if (!endClientSession) {
  ...
  try {
    ...
    if (requestedPortableExport) {
      ...
      const results: SeldonRunResult[] = finalizeRunResults([
        {
    ...
    const finalizedResults = finalizeRunResults(results);
    const message = parsed.message || (finalizedResults.length > 0 ? "Done — changes are live." : "Blueprint ready.");

    emitBrainEvent({
      status: "ok",
      action,
      results_count: finalizedResults.length,
      query_summary: rawDescription.slice(0, 140),
      context_source: contextMetrics.source,
      context_chars: contextMetrics.contextChars,
      context_selected_articles: contextMetrics.selectedArticles,
    ...
    const createdEntities = finalizedResults
      .map((result) => {
