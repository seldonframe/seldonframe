...
- [ ] **Step 4: Live smoke (after deploy, smoke-runner)** — assert per route: `/` 200 + "From your website" sentinel; `/?mode=record` 200 + `data-mode="record"` in the SSR HTML (no-flash contract); `/record` 200 + same sentinel + `robots` meta NOT noindex; `/record` with flag off (preview env) → 404; sitemap contains `/record`.
  - **Cross-host convergence (the merge's genuinely-new path):** the post-compile destination is `/studio/agents/<compiledTemplateId>`, driven by RecordClient state, NOT the entry URL — identical whether entered via the merged landing or app.seldonframe.com/record. This holds ONLY because www + apex + app are a single deployment (no host-split middleware, no www→app redirect — verified in vercel.json/next.config) and every link in the claim→compile→open-agent chain is relative, so the flow never leaves its origin host and the host-only NextAuth session cookie (no `domain=` set, auth.ts:208) stays valid throughout. Assert on the **www host specifically**: `www.seldonframe.com/signup` 200 (not a marketing-only 404) and `www.seldonframe.com/studio/agents/<any>` resolves to the dashboard shell (auth-redirect to /login is an acceptable 200-family signal that the route EXISTS on www; a hard 404 is a blocker — it would mean www/app are separate Vercel projects and the claim flow dead-ends after signup). If that ever 404s, the fix is to make the claim/open-agent links host-absolute to `app.seldonframe.com`, NOT to duplicate routes onto www.
