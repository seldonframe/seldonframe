...
## Approved decisions
- **Layout:** the **left keeps the hero photo** (the trade image) with the headline / subhead / trust / secondary **Call + Book** overlaid on a dark scrim; the **lead-form card sits on the right**. Proven Platinum-style high-converting layout, across all 3 hero variants. (Mobile: photo + headline stack, then the form.)
...
Child of `<Hero>`, rendered only when `leadForm.enabled`. Compact card (Platinum-style): **Name**, **Phone**, **"What do you need?"** (select from `needOptions`, else short text), submit button (default "Get a fast callback"), consent line under the button. `useTransition` + success state; imports `submitLeadFormAction` directly (mirrors `components/bookings/public-booking-form.tsx`). Props: `orgSlug`, `businessName`, `leadForm` config, `archetype` (for theming). **Placement: the right column of the hero**, across all 3 hero variants (`HeroSplit`, `HeroLeftAsymmetric`, `HeroCinematic`); the **left column retains the hero photo** with the headline / trust / Call + Book overlaid on a dark scrim for legibility. Styled per the archetype's CSS vars (`--primary`, `--font-headline`, radius) so it's world-class + on-brand, not a generic form. On mobile it stacks below the headline.
